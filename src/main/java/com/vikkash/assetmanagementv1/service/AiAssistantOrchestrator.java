...
public class AiAssistantOrchestrator {
    ...
    public AiAssistantOrchestrator(OpenAiResponsesClient openAiClient, AiToolExecutor toolExecutor,
    ...
    private void runTurn(String conversationId, List<Map<String, Object>> input, boolean isAdmin, String callerId, SseEmitter emitter) {
        ...
        for (int iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
            ...
            openAiClient.stream(input, tools, new OpenAiResponsesClient.StreamListener() {
                @Override
    ...
    private Map<String, Object> functionCall(String callId, String name, String argumentsJson) {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("type", "function_call");
        m.put("call_id", callId);
        m.put("name", name);
        m.put("arguments", argumentsJson);
        return m;
    }

    private Map<String, Object> functionCallOutput(String callId, Object result) {
        Map<String, Object> m = new LinkedHashMap<>();
        m.put("type", "function_call_output");
        m.put("call_id", callId);
        m.put("output", toJson(result));
        return m;
    }
    ...
    private void addHistoryItem(List<Map<String, Object>> input, AiConversationMessage past) {
        switch (past.getRole()) {
            ...
            default -> { /* "tool" rows are execution history for our own audit trail, not replayed
                            ...
                            original call_id pairing risks confusing the Responses API's tool-loop
                            validation, and the resulting *text* the model wrote already captured what
                            mattered for future turns. */ }
        }
    }
